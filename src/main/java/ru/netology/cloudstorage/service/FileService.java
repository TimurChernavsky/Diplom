...
public class FileService {
    ...
    @PostMapping("/file")
    public ResponseEntity<?> save(@RequestHeader("auth-token") String authToken,
                                  @RequestParam("filename") String fileName, MultipartFile file) throws IOException {
        String contentType = file.getContentType();
        long size = file.getSize();
        byte[] bytes = file.getBytes();
        save(authToken, fileName, contentType, size, bytes);
        return ResponseEntity.ok(HttpStatus.OK);
    }

    private void save(String authToken, String fileName, String contentType, long size, byte[] bytes) {
